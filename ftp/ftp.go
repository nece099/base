...
func Connect(addr string, timeout time.Duration, deadline time.Duration) (*ServerConn, error) {
	if timeout < 0 {
		timeout = 0
	}
	if deadline < 0 {
	...
	return DialTimeout(addr, timeout, deadline)
}
...
func DialTimeout(addr string, timeout time.Duration, deadline time.Duration) (*ServerConn, error) {
	...
	c.setFtpDeadline(c.netConn, c.deadline, true)
...
func (c *ServerConn) cmd(expected int, format string, args ...interface{}) (int, string, error) {
	c.setFtpDeadline(c.netConn, c.deadline, false)
...
func (c *ServerConn) NameList(path string) (entries []string, err error) {
	...
	c.setFtpDeadline(conn, c.deadline, true)
...
func (c *ServerConn) StorFrom(path string, r io.Reader, offset uint64) error {
	...
	c.setFtpDeadline(conn, c.deadline, false)
...
func (c *ServerConn) setFtpDeadline(conn net.Conn, dealline time.Duration, isRead bool) {
	if isRead {
		if d := dealline; d != 0 {
