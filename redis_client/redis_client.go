...
func NewRedisPool(c *RedisConfig) (pool *RedisPool) {
	...
	dialFunc := func() (rconn redis.Conn, err error) {
		...
		if err != nil {
			Log.Errorf("Redis connect %s error: %s", pool.env, err)
			return
		}

		if c.Password != "" {
			if _, err = rconn.Do("AUTH", c.Password); err != nil {
				Log.Errorf("Redis %s AUTH(password: %s) error: %s", pool.env, c.Password, err)
				rconn.Close()
				rconn = nil
				return
			}
		...
		if err != nil {
			Log.Errorf("Redis %s SELECT %s error: %s", pool.env, c.DBNum, err)
			rconn.Close()
			rconn = nil
		}
		return
